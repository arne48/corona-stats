...
CONFIRMED_PATH = get_file_from_link(DEFAULT_CONFIRMED_PATH)
DEATH_PATH = get_file_from_link(DEFAULT_DEATHS_PATH)
RECOVERED_PATH = get_file_from_link(DEFAULT_RECOVERED_PATH)

CONFIRMED_DATA, DATE_FIELDS = sanitize_data(pd.read_csv(CONFIRMED_PATH))
DEATH_DATA, DATE_FIELDS = sanitize_data(pd.read_csv(DEATH_PATH))
REOVERED_DATA, DATE_FIELDS = sanitize_data(pd.read_csv(RECOVERED_PATH))


def main():
    # Show single Country
    show_single_country('US', CONFIRMED_DATA, DATE_FIELDS, show_daily_increase=True, cases_in_log=False,
